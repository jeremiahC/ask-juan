...
const AGENTS = {
  ...
  // ── LEO ──────────────────────────────────────────────────────────────────
  'leo': {
    id: 'leo', icon: '💼', name: 'Leo', emoji: '🧑‍💼',
    role: 'Sales Specialist', domain: 'Sales',
    model: 'claude-opus-4-8',
    skills_prefix: 'sw:leo-sales-',
    tagline: 'From first client inquiry to signed proposal — Leo owns the pre-sales story.',
    desc: 'Leo handles the full pre-sales lifecycle: qualifying leads, building client proposals (WBS, cost, timeline in Excel + pitch deck) with real tech stack data from Carlo. He never invents numbers — every figure is grounded in real research.',
    tags: ['Lead Qualify', 'Proposal', 'WBS', 'Cost Estimate', 'Pitch Deck', 'Opus 4'],
    workflow: [
      { icon: '📩', label: 'Client Contact',   sub: 'Initial inquiry, RFP, or intro call' },
      { icon: '🔍', label: 'Qualify Lead',     sub: 'Discovery Q&A — problem, scope, budget' },
      { icon: '⚙️', label: 'Carlo Research',   sub: 'Stack, infra costs, third-party tools' },
      ...
      { icon: '✅', label: 'Client Approves',  sub: 'Handoff to Rica for project kickoff' },
    ],
    outputs: [
      {
        icon: '🔍', title: 'Qualify Brief',
        desc: 'Structured discovery brief capturing client problem, scope, and constraints.',
        preview: `# Client Qualification Brief — ABC Corp
Date: 2026-06-30

## Client
- Name / Company: Tanaka-san / ABC Corp
- Decision Maker: CTO
- Contact: tanaka@abccorp.jp

## Problem & Goals
- Problem: Manual delivery tracking losing ¥2M/mo in disputes
- End Users: 50 drivers + 5 dispatchers

## Scope
- Key Features: Real-time GPS, ERP integration, driver app
- Platform: Web (dispatch) + Android (drivers)
- Existing Assets: SAP ERP, no existing app

## Constraints
- Timeline: Go-live before Q4 2026
- Budget Range: ¥15–20M`,
      },
      {
        icon: '📊', title: 'Proposal Prep (Excel)',
        desc: 'Client-ready Excel with Q&A sheet, WBS, cost breakdown, Carlo\'s stack, and resource plan.',
        preview: `# Proposal Package — ABC Corp Logistics
Generated by Leo + Carlo | Date: 2026-06-30
...
Sheets:
  ...
  04_Stack        — Stack from Carlo: Next.js, Node, PostgreSQL, AWS
...
Next step: Share 00_QA with client → Leo updates
the full Excel once answers come back.`,
  ...
  3. Proposed Solution — Approach & tech stack (Carlo's rec)
...
Run: /sw:leo-sales-qualify → /sw:leo-sales-proposal-prep → /sw:leo-sales-proposal-draft`,
      },
    ],
    promptTrigger: '/sw:leo-sales-qualify',
...
Can you qualify this lead and prepare a proposal?`,
    sampleResponse: `On it! Let me start by qualifying the lead properly.

**Client:** ABC Corp (Tanaka-san, CTO)
**Problem:** Manual delivery tracking causing disputes

A few quick questions before I build the proposal:

1. What platform — web for dispatch, mobile for drivers, or both?
2. Is there a budget range in mind?
3. Do they have any existing designs or system references?

Once you answer, I'll pull Carlo in for the tech brief,
then generate the full proposal package.`,
    skills: [
      { icon: '🔍', name: 'Lead Qualifier',    desc: 'Discovery Q&A to understand client needs before any proposal.', cmd: '/sw:leo-sales-qualify' },
      { icon: '📊', name: 'Proposal Prep',     desc: 'Excel workbook: WBS, cost, timeline, Carlo\'s stack + infra.', cmd: '/sw:leo-sales-proposal-prep' },
      { icon: '🎯', name: 'Proposal Draft',    desc: '8-slide PPTX pitch deck derived from the prep Excel.', cmd: '/sw:leo-sales-proposal-draft' },
    ],
  },
  ...
  'rica': {
    ...
    tagline: 'From approved deal to sprint plan — Rica owns the full product story.',
    desc: 'Rica handles the PM lifecycle after a deal is confirmed: project charters, PRDs, and sprint timelines. She works without .juan/context.md at kickoff — that gets set up after Carlo\'s TRD is done.',
    tags: ['Charter', 'PRD', 'Sprint Plan', 'Opus 4'],
    workflow: [
      ...
      { icon: '⚙️', label: 'Carlo TRD',        sub: 'Carlo creates technical plan' },
      { icon: '🔧', label: 'juan:setup',       sub: 'Register project stack & constraints' },
      { icon: '📅', label: 'Sprint Timeline',  sub: 'Breaks PRD into sprints with estimates' },
      { icon: '✅', label: 'Handoff to Dev',   sub: 'Carlo spawns Nico + Rex' },
    ],
    outputs: [
      {
...
- US-05 Export report (5 pts)`,
    ...
    promptTrigger: '/sw:rica-pm-charter',
    samplePrompt: `The client confirmed — they want to proceed with the
logistics management app Leo proposed.

Key details from Leo's proposal:
- ~50 drivers, real-time GPS tracking
- SAP ERP integration
- Timeline: 6 months

Can you draft the project charter to kick things off?`,
    sampleResponse: `On it! Let me draft the project charter.

No .juan/context.md yet — that comes after Carlo's TRD.
I'll work from what we have and flag any gaps.

Reading Leo's qualify brief... ✓

Drafting charter now...

📋 plans/charter.md

Sections:
  1. Project Overview — Logistics fleet management for ABC Corp
  2. Goals — Real-time tracking, ERP sync, driver mobile app
  3. Scope (In/Out) — GPS + SAP integration in; payments out
  4. Key Stakeholders — Tanaka-san (CTO), Sun* PM: Rica
  5. Timeline Overview — 6 months, 3 phases
  6. Risks & Assumptions — SAP API access TBD
  7. Sign-off

Next: Run /sw:rica-pm-prd to write the PRD.`,
    skills: [
...
Building the tech plan now...`,
    skills: [
      { icon: '🔬', name: 'Pre-Sales Research', desc: 'Stack recommendation, infra costs, third-party tools for Leo\'s proposal.', cmd: '/sw:carlo-lead-research' },
...
const DOMAINS = [
  ...
  },
  {
    ...
    label: 'Sales',
    icon: '📈',
    desc: 'Lead qualification, client proposals, pitch decks, and deal handoff — Leo handles pre-sales end to end.',
    comingSoon: false,
  },
  {
    id: 'marketing',
...
const TEAM = [
  { id: 'leo',   emoji: '🧑‍💼', domain: 'Sales',         color: '#f59e0b', bizDomain: 'sales' },
